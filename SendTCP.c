...
#include <netinet/in.h> // struct sockddr_in    AF_INET    htons()
#include <sys/socket.h> // struct sockddr   socket()    connect()
...
int main(){
    ...
    inet_pton(AF_INET,
    "127.0.0.1", //loopback ip - ip reserved for internal networking
    &server.sin_addr);
    ...
    send(sockfd, "Hello from Receive", 18, 0);
